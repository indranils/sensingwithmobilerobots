...
void declareVariables(ofstream &ofp, int number_of_robots, int number_of_points)
{
  ...
  for (count1 = 0; count1 < number_of_robots; count1++)
  {
    for (count2 = 0; count2 < number_of_points - 1; count2++)
    {
  ...
  for (count1 = 0; count1 < number_of_robots; count1++)
  {
    for (count2 = 0; count2 < number_of_points; count2++)
    {
...
void writeInitialLocationConstraints(ofstream &ofp, pos_vec_t pos_start)
{
  unsigned int count1;

  for (count1 = 0; count1 < pos_start.size(); count1++)
  {
    ofp << "(assert (= x_" << count1 + 1 << "_1 " << pos_start[count1].x << "))" << endl;
    ofp << "(assert (= y_" << count1 + 1 << "_1 " << pos_start[count1].y << "))" << endl;
  }
  ofp << endl;
}


void writeObstacleConstraints(ofstream &ofp, dimension_t dimension, pos_vec_t obstacles, int number_of_robots, int number_of_points)
{
  unsigned int count, count1, count2;

  bool obs[dimension.length_x + 1][dimension.length_y + 1];

  for (count1 = 0; count1 <= dimension.length_x; count1++)
  {
    for (count2 = 0; count2 <= dimension.length_y; count2++)
    {
      obs[count1][count2] = false;
    }
  }
  
  for (count = 0; count < obstacles.size(); count++)
  {
    obs[obstacles[count].x][obstacles[count].y] = true;
  }
  for (count1 = 0; count1 <= dimension.length_x; count1++)
  {
    for (count2 = 0; count2 <= dimension.length_y; count2++)
    {
      if (obs[count1][count2] == true)
...
void writeTransitionConstraints(ofstream &ofp, prim_vec_t primitives, dimension_t dimension, pos_vec_t obstacles, int number_of_robots, int number_of_points)
{
  state q_i, q_f;
  position pos_f;
  ...
  for (count1 = 0; count1 < number_of_robots; count1++)
  {
    ...
    ofp << "(assert (= vel_f_" << count1 + 1 << "_" << number_of_points - 1 << " 0))" << endl;
  }
  ofp << endl;
  ...
  for (count1 = 0; count1 < number_of_robots; count1++)
  {
    for (count2 = 1; count2 < number_of_points - 1; count2++)
    {
  ...
  for (count1 = 0; count1 < number_of_robots; count1++)
  {
    for (count2 = 0; count2 < number_of_points - 2; count2++)
    {
  ...
  for (count1 = 0; count1 < number_of_robots; count1++)
  {
    for (count2 = 0; count2 < number_of_points; count2++)
    {
      ofp << "(assert (and (>= x_" << count1 + 1 << "_" << count2 + 1 << " 0) (<= x_" << count1 + 1 << "_" << count2 + 1 << " " << dimension.length_x << ")))" << endl;
    }
  }
  ofp << endl;
  ...
  for (count1 = 0; count1 < number_of_robots; count1++)
  {
    for (count2 = 0; count2 < number_of_points; count2++)
    {
      ofp << "(assert (and (>= y_" << count1 + 1 << "_" << count2 + 1 << " 0) (<= y_" << count1 + 1 << "_" << count2 + 1 << " " << dimension.length_y << ")))" << endl;
    }
  }
  ofp << endl;
  ...
  for (count1 = 0; count1 < number_of_robots; count1++)
  {
    for (count2 = 1; count2 < number_of_points; count2++)
    {
...
void writeCostConstraint(ofstream &ofp, dimension_t dimension, pos_vec_t obstacles, int number_of_robots, int number_of_points, float total_cost) 
{
  unsigned int count1, count2;
  ...
  /*for (count1 = 0; count1 <= dimension.length_x; count1++)
  {
    for (count2 = 0; count2 <= dimension.length_y; count2++)
    {
      ...
      {
          ofp << " penalty_" << count1 << "_" << count2;
      }
    }
  }
  ofp << endl;*/
  ofp << ")))" << endl;
  ofp << "(assert (<= total_cost " << floatToReal(to_string(total_cost)) << "))" << endl;
  ofp << endl;
}
...
void writeCollisionAvoidanceConstraints1(ofstream &ofp, prim_vec_t primitives, int number_of_robots, int  number_of_points)
{
  ...
  for (count = 0; count < number_of_points - 1; count++)
  {
    for (count1 = 0; count1 < number_of_robots; count1++)
    {
      for (count2 = 0; count2 < number_of_robots; count2++)
      {
        if (count1 != count2)
        {
...
void writeCollisionAvoidanceConstraints2(ofstream &ofp, prim_vec_t primitives, int number_of_robots, int number_of_points) 
{
...
void writeDistanceConstraints(ofstream &ofp, int number_of_robots, int number_of_points)
{
  unsigned int count, count1, count2;
  for (count = 1; count <= number_of_points; count++)
  {
    for (count1 = 1; count1 <= number_of_robots; count1++)
    {
      for (count2 = 1; count2 <= number_of_robots; count2++)
      {
        if (count1 != count2)
        {
...
void writeOutputConstraints(ofstream &ofp, int number_of_robots, int number_of_points)
{
  unsigned int count1, count2;
